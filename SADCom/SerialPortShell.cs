...
using SADCom.Configuration;

namespace SADCom {

	/// <summary>
	/// Fenetre principale représentant le terminal.
	/// </summary>
	public partial class SerialPortShell : Form {
		//private SerialPort mSerialPort;

		/// <summary>
		/// Managed the configuration of this sessions. Use by many class.
		/// </summary>
		...
		///<summary>
		/// Constructor. Initiate the form and start the configuration windows. 
		/// </summary>
		public SerialPortShell() {
			mSessionConfigurations = new SessionConfigurations();
			
			SerialPortConfiguration configForm = new SerialPortConfiguration(this.mSessionConfigurations);

			/// Initate the form
			InitializeComponent();

			///Call the configuration menu and, if return OK, configure and start the serial configuration, else close the form.
			try {
				if(configForm.ShowDialog() != DialogResult.OK) {
					this.Close();
				}
			} catch (Exception e) {
				MessageBox.Show("Erreur liée à la fenêtre de configuration :  " + e.ToString(), Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Error);
			}
